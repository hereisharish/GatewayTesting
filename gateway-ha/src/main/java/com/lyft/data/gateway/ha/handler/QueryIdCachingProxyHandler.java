...
public class QueryIdCachingProxyHandler extends ProxyHandler {
  ...
  public String rewriteTarget(HttpServletRequest request) {
    ...
    try {
    	log.info("query is here ==> " + CharStreams.toString(request.getReader()));
    	
        String text = IOUtils.toString(request.getInputStream(), StandardCharsets.UTF_8.name());
        
        log.info("Http Request original input stream in string " + text);
	} catch (IOException e) {
    ...
    if (isPathWhiteListed(request.getRequestURI())) {
      ...
      if (!Strings.isNullOrEmpty(queryId)) {
        ...
        String routingGroup = getRoutingGroupForInnovationJam(user);
  ...
  public String getRoutingGroupForInnovationJam(String user) {
	  
	  String routingGroup = null;
	  
	  if(user.equalsIgnoreCase("spiderman")) {
		  routingGroup = "marvel";
	  } else if(user.equalsIgnoreCase("superman")) {
		  routingGroup = "dc";
	  } else if(user.equalsIgnoreCase("ironman")) {
		  routingGroup = "marvel";
	  } else if(user.equalsIgnoreCase("batman")) {
		  routingGroup = "dc";
	  } else if(user.equalsIgnoreCase("thor")) {
		  routingGroup = "marvel";
	  } else if(user.equalsIgnoreCase("doctorstrange")) {
		  routingGroup = "marvel";
	  }
